...
def     RISCVCore(imem_data, dmem_data, rf, clock, reset, env):
    ...
    u_IM = Rom(sig.instruction, sig.PC, imem_data)
    ...
    u_PC_Src = And2(sig.PCSrc, sig.Branch, sig.Zero)
    u_mux_3 = Mux2(sig.NextPC, sig.PC4, sig.BranchTarget, sig.PCSrc)
    u_imm = ImmGen(sig.immediate, sig.instruction)
    u_reg = RegisterFile(sig.ReadData1 ,sig.ReadData2 ,sig.rs1, sig.rs2, sig.rd, sig.WriteData, sig.RegWrite, rf ,clock, True)
    u_mux_1 = Mux2(sig.ALUInput2, sig.ReadData2, sig.immediate, sig.ALUSrc)
    u_ALU = ALU(sig.ALUResult, sig.Zero, sig.ReadData1, sig.ALUInput2, sig.ALUOperation)
    u_ALU_Operation = ALUControl(sig.ALUOp, sig.instr30, sig.funct3, sig.ALUOperation)
    u_DM = Ram(sig.MemReadData, sig.ReadData2, sig.ALUResult, sig.MemRead, sig.MemWrite, dmem_data, clock)
    u_mux_2 = Mux2(sig.WriteData, sig.ALUResult, sig.MemReadData, sig.MemtoReg)

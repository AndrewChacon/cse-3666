...
binary_search: # add to the stack
	addi sp, sp, -8
	sw s1, 0(sp)
	sw ra, 4(sp)
	bne a1, x0, IF_STATMENT # check for empty array
	addi a0, x0, -1 
	beq x0, x0, F_EXIT

IF_STATMENT:
	srai t0, a1, 1 # dividing by 2
	slli t6, t0, 2
	add t1, a0, t6
	lw t5, 0(t1)
	
	bne t5, a2, ELSE_IF 
	addi a0, t0, 0 # increment rv
	beq x0, x0, F_EXIT

	
ELSE_IF: # (v < half_v) - searches the first half of array, recursive call 
	blt t5, a2, ELSE
	addi a1, t0, 0
	jal ra, binary_search
	beq x0, x0, F_EXIT
...
ELSE: # v > half_v - searches the send half of array, recursive call
	addi s1, t0, 1
	slli t6, s1, 2
	add a0, t6, a0
	sub a1, a1, s1
